//'use strict';
// generated on 2015-01-07 using generator-cg-gas 3.3.3
...
*/
  gulp.src('app/*.html')
    .pipe(wiredep({exclude: ['bootstrap-sass-official', 'vendor/d3']}))
    .pipe(gulp.dest('app'));
});

//build files for creating a dist release
gulp.task('build:dist', ['clean'], function(cb) {
  runSequence(['jshint', 'build', 'copy', 'copy:assets', 'images', 'font'], 'html', cb);
});
...
gulp.task('html', function() {
  ...
  .pipe($.if('**/*app.js', $.replace('/*templates_holder*/', ', "templates"')))
  ...
  .pipe($.useref())
  .pipe($.revReplace())
  .pipe($.if(['!index.html', '*.html'], $.minifyHtml({
    empty: true
  })))
